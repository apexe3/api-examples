# Stock Price Forecasting - Specification

## Overview
Build a stock price forecasting model using NeuralForecast's NHITS (Neural Hierarchical Interpolation for Time Series Forecasting) architecture to predict S&P 500 (^GSPC) stock prices for the next 5 days, leveraging the APEXE3 MCP server API endpoints for data retrieval.

## Project Structure
```
ForecastMLUseCase/
├── data_fetcher.py          # Fetch financial data from APEXE3 API
├── data_preprocessor.py     # Preprocess and prepare data for NeuralForecast
├── model_builder.py         # Build and configure NHITS model
├── trainer.py               # Train the model
├── forecaster.py            # Generate forecasts
├── evaluator.py             # Evaluate model performance
├── config.py                # Configuration parameters
├── requirements.txt         # Python dependencies
└── main.py                  # Main execution script
```

## Prerequisites
- Python 3.8+
- NeuralForecast
- NumPy
- Pandas
- PyTorch
- Requests
- Matplotlib

## Step-by-Step Implementation

DO NOT CLONE THE https://github.com/apexe3/api-examples repository

### Step 1: Setup Configuration (config.py)
Create a configuration file with the following parameters:
- API base URL: `https://alicev2.apexe3.ai/alice/apexe3/datahub/api`
- Default stock symbol: ^GSPC
- Date range for training data (5 years 2020 to December 31st 2024)
- NHITS hyperparameters:
  - Input size (lookback window)
  - Horizon (forecast period = 5 days)
  - Number of stacks
  - Number of layers per stack
  - Hidden layer sizes
  - Number of epochs
  - Batch size
  - Learning rate
- Train/validation split ratio

### Step 2: Data Fetcher (data_fetcher.py)
Implement a class to fetch historical price data from APEXE3 API:

**API Endpoint:**
```
GET https://alicev2.apexe3.ai/alice/apexe3/datahub/api/asset_price_history/{symbol}
Query Parameters:
  - start_date: YYYY-MM-DD (optional)
  - end_date: YYYY-MM-DD (optional)
```

**Requirements:**
- Create a `DataFetcher` class
- Method `fetch_historical_data(symbol, start_date, end_date)`:
  - Make HTTP GET request to APEXE3 API
  - Handle API errors and network issues
  - Return JSON response with OHLCV data
  - Parse response into pandas DataFrame
  - Validate data integrity
- Method `get_sp500_symbols()`:
  - Fetch S&P 500 symbols from APEXE3 API
  - Return list of available symbols

**Expected Response Format:**
```json
{
  "symbol": "^GSPC",
  "start_date": "YYYY-MM-DD",
  "end_date": "YYYY-MM-DD",
  "data": [
    {
      "date": "YYYY-MM-DD",
      "open": number,
      "high": number,
      "low": number,
      "close": number,
      "adj_close": number,
      "volume": integer
    }
  ]
}
```

### Step 3: Data Preprocessor (data_preprocessor.py)
Implement a class to prepare data for NeuralForecast training:

**Requirements:**
- Create a `DataPreprocessor` class
- Method `clean_data(df)`:
  - Handle missing values (forward fill or drop)
  - Remove outliers using IQR method
  - Ensure chronological order
- Method `create_features(df)`:
  - Calculate technical indicators:
    - Moving averages (5, 10, 20, 50 days)
    - RSI (Relative Strength Index)
    - MACD (Moving Average Convergence Divergence)
    - Bollinger Bands
    - Volume changes
  - Create lag features
- Method `prepare_neuralforecast_format(df)`:
  - Convert to NeuralForecast format
  - Required columns: 'unique_id', 'ds', 'y'
  - 'unique_id': stock symbol
  - 'ds': date column
  - 'y': target variable (close price)
- Method `split_data(df, test_ratio)`:
  - Split into train and validation sets
  - Maintain temporal order (no shuffling)

### Step 4: Model Builder (model_builder.py)
Implement NHITS model architecture:

**Requirements:**
- Create a `ModelBuilder` class
- Method `build_nhits_model(config)`:
  - Initialize NHITS model from NeuralForecast
  - Configure input size (lookback window)
  - Set horizon to 5 days
  - Configure number of stacks (typically 2-3)
  - Configure number of layers per stack
  - Set hidden layer sizes
  - Use Adam optimizer
  - Loss function: MSE or MAE
- Method `get_model_summary(model)`:
  - Print model architecture and parameters

**Model Architecture:**
```
NHITS Configuration:
  - Input size: 30-60 days (lookback window)
  - Horizon: 5 days
  - Stacks: 2-3
  - Layers per stack: 2-3
  - Hidden sizes: [256, 128, 64]
  - Pooling operations for multi-scale analysis
  - Max pooling for downsampling
  - Linear interpolation for upsampling
```

### Step 5: Trainer (trainer.py)
Implement model training logic:

**Requirements:**
- Create a `Trainer` class
- Method `train_model(model, train_df, val_df, config)`:
  - Train with early stopping callback
  - Use validation split (e.g., 0.2)
  - Implement learning rate reduction on plateau
  - Save training history
- Method `save_model(model, filepath)`:
  - Save trained model
- Method `load_model(filepath)`:
  - Load saved model
- Method `plot_training_history(history)`:
  - Visualize training and validation loss
  - Plot MAE over epochs

**Callbacks:**
- EarlyStopping: patience=10, restore_best_weights=True
- ReduceLROnPlateau: factor=0.5, patience=5

### Step 6: Forecaster (forecaster.py)
Implement forecasting functionality:

**Requirements:**
- Create a `Forecaster` class
- Method `generate_forecast(model, last_sequence, horizon)`:
  - Generate 5-day forecast
  - Use most recent sequence
  - Return predicted values with confidence intervals
- Method `plot_forecast(historical_data, forecast)`:
  - Visualize historical prices and forecast
  - Show confidence intervals
  - Display forecast dates and values
- Method `format_forecast_output(forecast)`:
  - Format forecast results for display
  - Include date, predicted price, and confidence bounds

### Step 7: Evaluator (evaluator.py)
Implement model evaluation metrics:

**Requirements:**
- Create an `Evaluator` class
- Method `calculate_metrics(y_true, y_pred)`:
  - Mean Absolute Error (MAE)
  - Mean Squared Error (MSE)
  - Root Mean Squared Error (RMSE)
  - Mean Absolute Percentage Error (MAPE)
  - R-squared (R²)
- Method `calculate_directional_accuracy(y_true, y_pred)`:
  - Percentage of correct directional predictions
- Method `generate_evaluation_report(metrics)`:
  - Create an advanced dashboard with all metrics
  - Include an actual price vs predicted price chart using the actual price units
  - Include model performance interpretation
  - Display 5-day forecast with confidence intervals

### Step 8: Main Execution Script (main.py)
Create the main orchestration script:

**Requirements:**
- Import all modules
- Load configuration
- Execute pipeline:
  1. Fetch data from APEXE3 API
  2. Preprocess data
  3. Split into train/validation
  4. Build NHITS model
  5. Train model
  6. Evaluate on validation set
  7. Generate 5-day forecast
  8. Generate report
  9. Save model and results
- Handle errors gracefully
- Log progress and results

**Execution Flow:**
```python
def main():
    # Load config
    config = load_config()
    
    # Fetch data
    fetcher = DataFetcher()
    df = fetcher.fetch_historical_data(config.symbol, config.start_date, config.end_date)
    
    # Preprocess
    preprocessor = DataPreprocessor()
    df_clean = preprocessor.clean_data(df)
    df_features = preprocessor.create_features(df_clean)
    df_nf = preprocessor.prepare_neuralforecast_format(df_features)
    train_df, val_df = preprocessor.split_data(df_nf, config.test_ratio)
    
    # Build model
    builder = ModelBuilder()
    model = builder.build_nhits_model(config)
    
    # Train
    trainer = Trainer()
    history = trainer.train_model(model, train_df, val_df, config)
    
    # Forecast
    forecaster = Forecaster()
    forecast = forecaster.generate_forecast(model, train_df, config.horizon)
    
    # Evaluate
    evaluator = Evaluator()
    metrics = evaluator.calculate_metrics(val_df['y'], forecast['y_hat'])
    
    # Save results
    trainer.save_model(model, 'nhits_model.pkl')
    evaluator.generate_evaluation_report(metrics)
    forecaster.plot_forecast(df_clean, forecast)
```

### Step 9: Requirements File (requirements.txt)
Create requirements.txt with:
```
neuralforecast>=1.6.0
torch>=2.0.0
numpy>=1.21.0
pandas>=1.3.0
requests>=2.26.0
matplotlib>=3.4.0
```

## API Integration Notes

### APEXE3 API Endpoints Used:
1. **Historical Price Data:**
   - Endpoint: `/asset_price_history/{symbol}`
   - Method: GET
   - Parameters: start_date, end_date
   - Returns: OHLCV data

2. **S&P 500 Symbols:**
   - Endpoint: `/sp500_symbols`
   - Method: GET
   - Returns: List of S&P 500 tickers

### Error Handling:
- Implement retry logic for API failures
- Handle rate limiting
- Validate API responses
- Log all API calls and errors
- Include a .gitignore which includes the venv entry

## Data Considerations

### Feature Engineering:
- Use closing price as primary target
- Include volume as feature
- Add technical indicators for better predictions
- Consider market sentiment if available

### Time Series Specifics:
- Maintain temporal order
- No data leakage from future
- Use time-based validation split
- Consider multiple time horizons (1-day, 5-day, 10-day)

## Model Optimization

### Hyperparameter Tuning:
- Input size: 30-90 days
- Number of stacks: 2-4
- Layers per stack: 2-3
- Hidden sizes: [128, 64, 32] to [512, 256, 128]
- Learning rate: 0.001-0.0001
- Batch size: 16-64

### Regularization:
- Dropout layers
- Early stopping
- Learning rate scheduling
- Weight decay

## Testing and Validation

### Validation Strategy:
- Time-based split (not random)
- Walk-forward validation
- Out-of-sample testing
- Multiple stock symbols

### Performance Benchmarks:
- MAPE < 5% for good model
- Directional accuracy > 55%
- Consistent performance across symbols

## Deployment Considerations

### Model Persistence:
- Save model architecture and weights
- Store scaler parameters
- Version control models
- Document model performance

### Monitoring:
- Track prediction accuracy over time
- Monitor model drift
- Retrain periodically with new data
- Alert on performance degradation

## Future Enhancements

1. **Multi-Stock Models:**
   - Train on multiple stocks
   - Use global models
   - Transfer learning

2. **Advanced Features:**
   - News sentiment analysis
   - Social media data
   - Macroeconomic indicators
   - Sector correlations

3. **Model Variants:**
   - NBEATS model
   - Temporal Fusion Transformer
   - DeepAR
   - Ensemble methods

4. **Production Features:**
   - Real-time inference API
   - Batch prediction pipeline
   - Automated retraining
   - A/B testing framework

## Success Criteria

- [ ] Successfully fetch data from APEXE3 API
- [ ] Clean and preprocess financial data
- [ ] Build functional NHITS model
- [ ] Train model without overfitting
- [ ] Generate accurate 5-day forecast
- [ ] Achieve reasonable prediction accuracy
- [ ] Generate comprehensive evaluation report
- [ ] Save and load model correctly
- [ ] Handle errors gracefully
- [ ] Document code and usage
- [ ] Create reproducible pipeline
